...
/* Hybrid Mobility Detection Variables */
static uint16_t t_elapsed;
static uint16_t t_mc;
static uint8_t hybrid_is_fixed;
...
handle_rl_monitoring(void *ptr)
{
  ...
  /* --- HYBRID MOBILITY ALGORITHM TICK --- */
  if(t_elapsed < 0xFFFF) {
    t_elapsed++;
  }

  /* Calculate EWMA: t_mc = (ALPHA * t_mc + (100 - ALPHA) * t_elapsed) / 100 */
  uint32_t mc_val = ((uint32_t)RIMS_M_TIMER_ALPHA * t_mc 
                  + (100 - RIMS_M_TIMER_ALPHA) * t_elapsed) / 100;
  t_mc = (uint16_t)mc_val;

  /* Absolute Stability Promotion & Standard Threshold Check */
  if(t_elapsed >= RIMS_MAX_STATIC_TICKS) {
    t_mc = RIMS_MAX_STATIC_TICKS;
    hybrid_is_fixed = 1;
  } else if(t_mc < RIMS_MOBILITY_THRESHOLD) {
    hybrid_is_fixed = 0;
  } else {
    hybrid_is_fixed = 1;
  }
...
rims_init(void)
{
  ...
  t_elapsed = 0;
  t_mc = RIMS_MAX_STATIC_TICKS; /* Start assuming static */
  hybrid_is_fixed = 1;
...
rims_notify_parent_change(void)
{
  ...
  /* Fast-Sprint Demotion: if multiple rapid switches happen, instantly drop t_mc */
  if (t_elapsed < RIMS_FAST_DROP_TICKS) {
     t_mc = 0;
     hybrid_is_fixed = 0;
  }
...
rims_is_fixed(void)
{
  rpl_instance_t *instance = default_instance;
  if(instance != NULL && instance->current_dag != NULL) {
    /* The DAG root is always considered fixed infrastructure */
    if(instance->current_dag->rank == ROOT_RANK(instance)) {
      return 1;
    }
  }
  return hybrid_is_fixed;
}
