...
/*
 * Seuil de proximite ETX (MRHOF, RFC6719 default = 96, eq ETX 0.75).
 * Si |pc1 - pc2| <= MRHOF_PC_THRESHOLD => les deux chemins sont equivalents
 * du point de vue ETX => TAU decide.
 */
...
/*---------------------------------------------------------------------------*/
/**
 * \brief Selectionne le meilleur parent - logique combinee MRHOF + TAU.
 *
 * Principe :
 *   MRHOF garantit qu'on ne degrade pas le chemin global vers la racine
 *   en choisissant un parent avec un bon TAU local mais un mauvais path-cost ETX.
 *   TAU affine la decision quand les path-costs sont equivalents ou quand
 *   la qualite globale du chemin compense l'ecart ETX.
 *
 * Filtres successifs :
 * 1. Liens inutilisables rejetes (ETX > MAX ou TAU < MIN).
 * 2. Rank constraint (loop-free DODAG garantit).
 * 3. Path-cost ETX cumule (MRHOF) compare au seuil MRHOF_PC_THRESHOLD.
 *    - Si proches => TAU tranche (avec hysteresis TAU).
 *    - Si ecart significatif => meilleur ETX l'emporte,
 *      SAUF si TAU adverse compense (delta > RPL_OF_TAU_SWITCH_THRESHOLD).
 * 4. Tie-breaks : path-cost ETX, rank, ETX direct.
 */
static rpl_parent_t *
best_parent(rpl_parent_t *p1, rpl_parent_t *p2)
{
  ...
  if(dag != NULL && (p1 == dag->preferred_parent || p2 == dag->preferred_parent)) {
     ...
     * Appliquer l'hysteresis combinee MRHOF + TAU.
     */
  ...
  /* --- CAS : pas de preferred_parent => selection directe MRHOF + TAU --- */
  {
  ...
  /* Tie-break 1: prefer lower path cost ETX (MRHOF) */
