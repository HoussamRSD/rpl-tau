...
/* Secret 4: Squared ETX penalty — compile-time flag for A/B testing */
...
calculate_candidate_score(rpl_parent_t *p)
{
...
#if RPL_OF_TAU_SQUARED_ETX
  /* Secret 4: Apply squared-ETX penalty before normalization.
   ...
   * This strongly penalizes marginal links and keeps the node on reliable parents. */
  {
    ...
    if(stats != NULL && stats->etx > 0) {
      ...
      uint16_t etx_penalized = (uint16_t)((etx_sq > 0xFFFF) ? 0xFFFF : etx_sq);
      /* Normalize penalized ETX to [0, 1000]: perfect=1→1000, ≥8→0 */
      if(etx_penalized <= LINK_STATS_ETX_DIVISOR) {
        fresh_etx = 1000;
      } else if(etx_penalized >= 8 * LINK_STATS_ETX_DIVISOR) {
        fresh_etx = 0;
      } else {
        uint32_t range = (8 * LINK_STATS_ETX_DIVISOR) - LINK_STATS_ETX_DIVISOR;
        fresh_etx = (uint16_t)(1000UL - ((etx_penalized - LINK_STATS_ETX_DIVISOR) * 1000UL / range));
      }
    } else {
      fresh_etx = 500;  /* Unknown link, use default */
    }
  ...
  if(default_instance != NULL && default_instance->current_dag != NULL) {
    ...
    if(p != NULL) {
      ...
      if(live_rssi > (int16_t)RL_RSSI_THRESHOLD &&
        ...
        rpl_rl_on_dio_received(dag, p, live_rssi);
      }
    }
  }
...
best_parent(rpl_parent_t *p1, rpl_parent_t *p2)
{
  ...
  if(dag != NULL && (p1 == dag->preferred_parent || p2 == dag->preferred_parent)) {
     ...
     * Secret 1: Dual-axis hysteresis — the candidate must beat the current
     * preferred parent on BOTH path-cost ETX AND TAU before a switch is allowed.
     */
    ...
    /* Dual-axis hysteresis check:
     * pc_advantage  = candidate path-cost is better by more than MRHOF_PC_THRESHOLD
     * tau_advantage = candidate TAU is better by more than RPL_OF_TAU_SWITCH_THRESHOLD
    ...
    return cand;     /* At least one axis clearly better — switch */
  }
