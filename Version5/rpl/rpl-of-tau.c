...
best_parent(rpl_parent_t *p1, rpl_parent_t *p2)
{
  ...
  if(dag != NULL && (p1 == dag->preferred_parent || p2 == dag->preferred_parent)) {
    ...
    if(pref == p1) {
      if(t2 <= (uint16_t)(t1 + RPL_OF_TAU_SWITCH_THRESHOLD)) {
        /* p1 wins by hysteresis, p2 is runner-up (backup) */
        rpl_set_backup_parent(p2);
        return p1;
      }
    } else if(pref == p2) {
      if(t1 <= (uint16_t)(t2 + RPL_OF_TAU_SWITCH_THRESHOLD)) {
        /* p2 wins by hysteresis, p1 is runner-up (backup) */
        rpl_set_backup_parent(p1);
        return p2;
      }
    }
  }
  ...
  if(t2 > t1) { winner = p2; loser = p1; }
  else if(t1 > t2) { winner = p1; loser = p2; }
  else {
    /* Tie-break 1: prefer lower rank (closer to root = fewer hops) */
    if(r2 < r1) { winner = p2; loser = p1; }
    else if(r1 < r2) { winner = p1; loser = p2; }
    else {
      /* Tie-break 2: prefer lower ETX (better direct link) */
      if(parent_link_metric(p2) < parent_link_metric(p1)) { winner = p2; loser = p1; }
      else { winner = p1; loser = p2; }
    }
  }

  /* MA-RPL: The loser of each comparison becomes the backup parent.
   * Since best_parent() is called pairwise during neighbor scan,
   * the final backup will be the best runner-up overall. */
  rpl_set_backup_parent(loser);
  return winner;
}
/*---------------------------------------------------------------------------*/
static rpl_dag_t *
