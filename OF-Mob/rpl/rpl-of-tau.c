...
compute_path_metric(rpl_parent_t *p)
{
   ...
   * Note: if path_erp < rssi, the subtraction term is negative,
   *       which INCREASES score (worse). Using int32_t for safety.
   */
...
best_parent(rpl_parent_t *p1, rpl_parent_t *p2)
{
  ...
  /*--- Priority Rule: Fixed nodes are preferred over mobile nodes ---*/
  if(p1->is_fixed && !p2->is_fixed) {
    PRINTF("RPL: OF-TAU selecting p1 (fixed over mobile)\n");
    return p1;
  }
  if(p2->is_fixed && !p1->is_fixed) {
    PRINTF("RPL: OF-TAU selecting p2 (fixed over mobile)\n");
    return p2;
  }

  /*--- Both same mobility class: use composite metric with hysteresis ---*/

  /* Maintain stability of the preferred parent */
  if(p1 == dag->preferred_parent || p2 == dag->preferred_parent) {
    /* Only switch if the improvement exceeds the hysteresis threshold */
    if(p1->path_metric < p2->path_metric + RPL_OF_TAU_SWITCH_THRESHOLD &&
       p1->path_metric > p2->path_metric - RPL_OF_TAU_SWITCH_THRESHOLD) {
